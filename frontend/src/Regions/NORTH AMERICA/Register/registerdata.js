...
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwGFdeKCHbuPQgo8yslQP6JiTn93cBliyp89ZhD3B6CCBTcsyeAySlTvOp5PHJHDtaW3A/exec";

const ADMIN_EMAIL = "northamericasignatureglobalcon@gmail.com";
...
export const getConferencesForRegion = (id) =>
  !id || id === "all" ? conferences : getConferencesByRegion(id);
...
export const INITIAL_FORM = {
  ...
  organization: "", jobTitle: "", regionId: "", conferenceId: "",
  speakerType: "", packageId: "", companions: 0, extraNights: 0,
...
// ─────────────────────────────────────────────────────────────
// EMAIL 1 — Confirmation to the registrant (clean, no emojis)
// ─────────────────────────────────────────────────────────────
const sendUserConfirmation = (p) => {
  ...
  const body = [
    ...
    "  northamericasignatureglobalcon@gmail.com",
    "------------------------------------------------------------",
    "",
...
// ─────────────────────────────────────────────────────────────
// EMAIL 2 — Notification to admin (clean, no emojis)
// ─────────────────────────────────────────────────────────────
const sendAdminNotification = (p) => {
  ...
  const body = [
    ...
    "",
    "------------------------------------------------------------",
    "  North America Signature Global Conferences — Registration System",
    "------------------------------------------------------------",
  ].join("\n");

  // replyTo = registrant email so you can reply directly from your inbox
  return sendEmail({ to: ADMIN_EMAIL, subject, body, replyTo: p.Email });
};

// ─────────────────────────────────────────────────────────────
// SUBMIT REGISTRATION
// Sends both emails in parallel — no Sheet logging
// ─────────────────────────────────────────────────────────────
export const submitRegistration = async (fields, allConferences) => {
  ...
  await Promise.all([
    ...
    sendAdminNotification(payload),  // -> northamericasignatureglobalcon@gmail.com
  ]);
