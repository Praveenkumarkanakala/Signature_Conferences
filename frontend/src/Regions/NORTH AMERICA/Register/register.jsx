...
function CountryDropdown({ value, onChange }) {
  ...
  return (
    <div className="na-rg-country-dropdown" ref={wrapRef}>
      <div className={`na-rg-country-trigger${open ? " na-rg-country-trigger--open" : ""}`}
        onClick={handleOpen} role="button" tabIndex={0}
        onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && handleOpen()}
        aria-expanded={open} aria-label="Select country code">
        {open ? (
          <input ref={inputRef} className="na-rg-country-search" value={query}
            onChange={(e) => setQuery(e.target.value)} placeholder="Search country…"
        ...
        <svg className={`na-rg-country-chevron${open ? " na-rg-country-chevron--up" : ""}`} viewBox="0 0 10 6" fill="none" aria-hidden="true">
          <path d="M1 1l4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      </div>
      {open && (
        <div className="na-rg-country-list" role="listbox">
          {filtered.length === 0 ? <div className="na-rg-country-empty">No countries found</div> : filtered.map((c) => (
...
const Field = ({ label, required, error, full, children }) => (
  <div className={`na-rg-field${full ? " na-rg-field--full" : ""}`}>
    <label className="na-rg-label">{label} {required && <span className="na-rg-required" aria-hidden="true">*</span>}</label>
    {children}
...
const ChevronSVG = () => (
  <svg viewBox="0 0 10 6" fill="none" width="10" height="6"><path d="M1 1l4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
);
    ...
    <div className="na-rg-step-indicator" role="navigation" aria-label="Registration steps">
      {steps.map((s, i) => (
        <div key={s.num} className="na-rg-step-indicator__item">
          <div className={`na-rg-step-indicator__circle ${currentStep > s.num ? "done" : currentStep === s.num ? "active" : ""}`}
          ...
          <span className={`na-rg-step-indicator__label ${currentStep === s.num ? "active" : ""}`}>{s.label}</span>
          {i < steps.length - 1 && <div className={`na-rg-step-indicator__line ${currentStep > s.num ? "done" : ""}`} aria-hidden="true" />}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <section className="na-rg-hero" aria-label="Registration hero">
      ...
      <div className="na-rg-hero__content">
        ...
        <h1 className="na-rg-hero__title">Claim Your <em>Global Stage</em></h1>
        <p className="na-rg-hero__sub">Register now to secure your speaking slot at one of our world-class conferences. Limited seats available — act fast.</p>
        <div className="na-rg-hero__stats" aria-label="Conference statistics">
          {[{ num: "40+", label: "Countries" }, { num: "2K+", label: "Speakers" }, { num: "12", label: "Conferences" }].map(({ num, label }) => (
...
function Step1({ fields, errors, set, setField }) {
  const conferences = useMemo(() => fields.regionId ? getConferencesForRegion(fields.regionId) : [], [fields.regionId]);
  const handleRegionChange = (e) => { setField("regionId", e.target.value); setField("conferenceId", ""); };

  return (
    <div className="na-rg-form-body">
      <div className="na-rg-section-label"><span className="na-rg-section-label__icon" aria-hidden="true">01</span><span>Personal Information</span></div>

      <div className="na-rg-row">
        <Field label="First Name" required error={errors.firstName}>
          <input value={fields.firstName} onChange={set("firstName")} className={`na-rg-input${errors.firstName ? " na-rg-input--error" : ""}`} placeholder="Jane" autoComplete="given-name" aria-required="true" aria-invalid={!!errors.firstName} />
        </Field>
        <Field label="Last Name" required error={errors.lastName}>
          <input value={fields.lastName} onChange={set("lastName")} className={`na-rg-input${errors.lastName ? " na-rg-input--error" : ""}`} placeholder="Smith" autoComplete="family-name" aria-required="true" aria-invalid={!!errors.lastName} />
        </Field>
      </div>

      <div className="na-rg-row">
        <Field label="Email Address" required error={errors.email}>
          <input value={fields.email} onChange={set("email")} type="email" className={`na-rg-input${errors.email ? " na-rg-input--error" : ""}`} placeholder="jane@example.com" autoComplete="email" aria-required="true" aria-invalid={!!errors.email} />
        </Field>
        <Field label="Phone Number" required error={errors.phone}>
          <div className="na-rg-phone-wrap">
            <CountryDropdown value={fields.countryCode} onChange={(code) => setField("countryCode", code)} />
            <input value={fields.phone} onChange={set("phone")} className={`na-rg-input na-rg-input--phone${errors.phone ? " na-rg-input--error" : ""}`} placeholder="Phone number" autoComplete="tel-national" type="tel" aria-required="true" aria-invalid={!!errors.phone} />
          </div>
        </Field>
      </div>
      ...
      <div className="na-rg-row">
        <Field label="Country of Residence" required error={errors.country}>
          <input value={fields.country} onChange={set("country")} className={`na-rg-input${errors.country ? " na-rg-input--error" : ""}`} placeholder="e.g. United States" autoComplete="country-name" aria-required="true" aria-invalid={!!errors.country} />
        </Field>
        <Field label="Organization / Company">
          <input value={fields.organization} onChange={set("organization")} className="na-rg-input" placeholder="Company / Brand" autoComplete="organization" />
        </Field>
      </div>

      <div className="na-rg-row na-rg-row--single">
        <Field label="Job Title / Role">
          <input value={fields.jobTitle} onChange={set("jobTitle")} className="na-rg-input" placeholder="CEO, Coach, Author…" autoComplete="organization-title" />
        </Field>
      </div>

      <div className="na-rg-section-label na-rg-section-label--spaced"><span className="na-rg-section-label__icon" aria-hidden="true">02</span><span>Conference Selection</span></div>

      <div className="na-rg-row">
        <Field label="Region" required error={errors.regionId}>
          <div className="na-rg-select-wrap">
            <select value={fields.regionId} onChange={handleRegionChange} className={`na-rg-select${errors.regionId ? " na-rg-select--error" : ""}`} aria-required="true" aria-invalid={!!errors.regionId}>
        ...
        <Field label="Conference" required error={errors.conferenceId}>
          <div className="na-rg-select-wrap">
            <select value={fields.conferenceId} onChange={set("conferenceId")} className={`na-rg-select${errors.conferenceId ? " na-rg-select--error" : ""}${!fields.regionId ? " na-rg-select--disabled" : ""}`} disabled={!fields.regionId} aria-required="true" aria-invalid={!!errors.conferenceId}>
...
function Step2({ fields, errors, setField }) {
  ...
  const CheckSVG    = () => <svg viewBox="0 0 12 10" fill="none" width="10" height="8"><path d="M1 5l3.5 3.5L11 1" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>;
  ...
  return (
    <div className="na-rg-form-body">
      {/* Participation Type */}
      <div>
        <div className="na-rg-section-label"><span className="na-rg-section-label__icon" aria-hidden="true">01</span><span>Participation Type</span></div>
        {errors.speakerType && <span className="na-rg-error-msg" role="alert" style={{ marginTop: 8, display: "block" }}>{errors.speakerType}</span>}
        <div className="na-rg-type-toggle" role="radiogroup" aria-label="Participation type">
          {[
            ...
            <button key={type} type="button"
              className={`na-rg-type-btn${fields.speakerType === type ? ` na-rg-type-btn--active${virtual ? " na-rg-type-btn--virtual" : ""}` : ""}`}
              onClick={() => handleTypeSwitch(type)} role="radio" aria-checked={fields.speakerType === type}>
              ...
              {fields.speakerType === type && <span className="na-rg-type-btn__check" aria-hidden="true"><CheckSVG /></span>}
            </button>
          ))}
        </div>
      </div>
      ...
      {fields.speakerType && (
        <div>
          <div className="na-rg-section-label na-rg-section-label--spaced"><span className="na-rg-section-label__icon" aria-hidden="true">02</span><span>Choose Your Package</span></div>
          {errors.packageId && <span className="na-rg-error-msg" role="alert" style={{ marginBottom: 12, display: "block" }}>{errors.packageId}</span>}
          <div className={`na-rg-pkg-grid${isVirtual ? " na-rg-pkg-grid--virtual" : ""}`} role="radiogroup" aria-label="Package options">
            {activePkgs.map((pkg) => (
              <div key={pkg.id} className={`na-rg-pkg-card${fields.packageId === pkg.id ? " na-rg-pkg-card--active" : ""}`}
                onClick={() => setField("packageId", pkg.id)} role="radio" aria-checked={fields.packageId === pkg.id}
                ...
                <div className="na-rg-pkg-card__header">
                  ...
                  <div className="na-rg-pkg-card__price" aria-label={`Price: $${pkg.price.toLocaleString()}`}>${pkg.price.toLocaleString()}</div>
                </div>
                ...
                <div className="na-rg-pkg-card__radio" aria-hidden="true">
                  {fields.packageId === pkg.id && <svg viewBox="0 0 12 10" fill="none" width="10" height="8"><path d="M1 5l3.5 3.5L11 1" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                </div>
              </div>
            ))}
          </div>
      ...
      {fields.speakerType === "physical" && (
        <>
          <div className="na-rg-section-label na-rg-section-label--spaced"><span className="na-rg-section-label__icon" aria-hidden="true">03</span><span>Additional Attendees &amp; Nights</span></div>
          {[
            ...
            <div key={key} className="na-rg-companion-box">
              <div className="na-rg-companion-box__info">
                <div className="na-rg-companion-box__title"><span className="na-rg-companion-box__title-icon" aria-hidden="true">{icon}</span>{title}</div>
                <div className="na-rg-companion-box__sub" dangerouslySetInnerHTML={{ __html: sub.replace(/\$\d+/, (m) => `<strong>${m}</strong>`) }} />
              </div>
              <div className="na-rg-companion-counter" role="group" aria-label={`Number of ${key}`}>
                <button className="na-rg-counter-btn" disabled={(fields[key] || 0) === 0} onClick={() => setField(key, Math.max(0, (fields[key] || 0) - 1))} type="button" aria-label={`Decrease ${key}`}>−</button>
                <span className="na-rg-counter-val" aria-live="polite">{fields[key] || 0}</span>
                <button className="na-rg-counter-btn" onClick={() => setField(key, (fields[key] || 0) + 1)} type="button" aria-label={`Increase ${key}`}>+</button>
              </div>
            </div>
          ))}
        </>
      ...
      <PriceSummaryBar selectedPkg={selectedPkg} companions={fields.companions} extraNights={fields.extraNights || 0} isVirtual={isVirtual} total={total} discount={fields.discount} label="Estimated Total" />
    </div>
  );
}
...
function PriceSummaryBar({ selectedPkg, companions, extraNights = 0, isVirtual, total, discount, label, couponCode }) {
  ...
  return (
    <div className="na-rg-total-bar" aria-label="Price summary">
      ...
      <div className="na-rg-total-bar__amount"><span aria-label={`Total: $${total.toLocaleString()}`}>${total.toLocaleString()}</span></div>
    </div>
  );
}
      ...
      {status === "applied" ? (
        <div className="na-rg-coupon-applied" role="status" aria-live="polite">
          <span className="na-rg-coupon-applied__tag">
            <svg viewBox="0 0 16 16" fill="none" width="14" height="14" aria-hidden="true"><circle cx="8" cy="8" r="7" stroke="currentColor" strokeWidth="1.5"/><path d="M5 8l2 2 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            <strong>{couponCode}</strong> — ${discount} off applied!
          </span>
          <button type="button" className="na-rg-coupon-remove" onClick={handleRemove} aria-label="Remove coupon">Remove</button>
        </div>
      ) : (
        <div className={`na-rg-coupon-row${shake ? " na-rg-coupon-row--shake" : ""}`}>
          <input value={input} onChange={(e) => { setInput(e.target.value.toUpperCase()); setStatus("idle"); }}
            ...
            placeholder="ENTER CODE" maxLength={20} aria-label="Coupon code" aria-invalid={status === "error"} />
          <button type="button" className="na-rg-coupon-btn" onClick={handleApply} disabled={!input.trim()} aria-label="Apply coupon code">Apply</button>
        </div>
      )}
      {status === "error" && <span className="na-rg-error-msg" role="alert" style={{ marginTop: 6, display: "block" }}>Invalid coupon code. Please try again.</span>}
    </div>
  );
}
...
function Step3({ fields, allConferences, onEditStep, setField }) {
  ...
  const total     = calculateTotal(fields.packageId, fields.companions, fields.discount || 0, fields.extraNights || 0);
  const region    = REGIONS.find((r) => r.id === fields.regionId)?.label || "—";
  const isVirtual = fields.speakerType === "virtual";
      ...
      <ReviewSection title="Package, Companions &amp; Nights" onEdit={() => onEditStep(2)}>
        ...
        {!isVirtual && (
          <div className="na-rg-review-grid" style={{ marginTop: 14 }}>
            <ReviewRow label="Companions" value={fields.companions === 0 ? "None" : `${fields.companions} person${fields.companions > 1 ? "s" : ""} (+$${fields.companions * COMPANION_PRICE})`} />
            <ReviewRow label="Extra Nights" value={(fields.extraNights || 0) === 0 ? "None" : `${fields.extraNights} night${fields.extraNights > 1 ? "s" : ""} (+$${fields.extraNights * EXTRA_NIGHT_PRICE})`} />
          </div>
        )}
      </ReviewSection>

      <CouponWidget couponCode={fields.couponCode} discount={fields.discount}
      ...
      <div className="na-rg-total-bar na-rg-total-bar--final" aria-label="Final amount">
        <div className="na-rg-total-bar__left">
          ...
          <div className="na-rg-total-bar__breakdown">
            {fields.discount > 0 ? `Coupon "${fields.couponCode}" saves you $${fields.discount}` : "Our team will contact you to confirm payment details"}
          </div>
        </div>
        <div className="na-rg-total-bar__amount">
          {fields.discount > 0 && pkg && (
            <span className="na-rg-total-bar__original">${calculateTotal(fields.packageId, fields.companions, 0, fields.extraNights || 0).toLocaleString()}</span>
          )}
  ...
  <div className="na-rg-review-section">
    <div className="na-rg-review-section__head">
      ...
      <button className="na-rg-edit-btn" onClick={onEdit} type="button" aria-label={`Edit ${title}`}>
        <svg viewBox="0 0 14 14" fill="none" width="11" height="11" aria-hidden="true"><path d="M9.5 1.5l3 3L4 13H1v-3L9.5 1.5z" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
        Edit
      </button>
    </div>
    {children}
  ...
  return (
    <section className="na-rg-result na-rg-result--success" role="main" aria-label="Registration successful">
      <div className="na-rg-result__confetti" aria-hidden="true">
        {Array.from({ length: 18 }).map((_, i) => <span key={i} className="na-rg-result__confetti-piece" style={{ "--i": i }} />)}
      </div>
      <div className="na-rg-result__card">
        ...
        <div className="na-rg-email-notice" aria-label="Email confirmation">
          ...
          <div>
            ...
            <div className="na-rg-email-notice__sub">A detailed confirmation has been sent to <strong>{fields.email}</strong>. Please check your inbox (and spam folder).</div>
          </div>
        </div>
...
const FailScreen = ({ onRetry }) => (
  <section className="na-rg-result na-rg-result--fail" role="main" aria-label="Registration failed">
    <div className="na-rg-result__card">
      ...
      <p className="na-rg-result__sub">Something went wrong. Your details have been saved — please try again. If the issue persists, contact us directly.</p>
      <div className="na-rg-result__actions">
...
function RegistrationForm({ onSuccess, onFail }) {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try { await submitRegistration(fields, allConferences); onSuccess(fields); }
    catch (err) { console.error("Submission error:", err); onFail(); }
  ...
  return (
    <div className="na-rg-form-card" role="main">
      ...
      {step === 3 && <Step3 fields={fields} allConferences={allConferences} onEditStep={goToStep} setField={setField} />}

      <div className="na-rg-form-footer">
        <div className="na-rg-form-footer__actions">
          {step > 1 && (
            <button className="na-rg-back-btn" onClick={goBack} type="button" aria-label="Go back">
              <svg viewBox="0 0 16 10" fill="none" width="14" height="10" aria-hidden="true"><path d="M5 1L1 5l4 4M1 5h14" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
              Back
            </button>
          )}
          {step < 3 ? (
            <button className="na-rg-submit-btn" onClick={goNext} type="button">
              Continue
              <svg viewBox="0 0 16 10" fill="none" width="14" height="10" aria-hidden="true"><path d="M11 1l4 4-4 4M15 5H1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
          ) : (
            <button className="na-rg-submit-btn" onClick={handleSubmit} disabled={submitting} type="button" aria-busy={submitting}>
              {submitting
                ? <><span className="na-rg-spinner" aria-hidden="true" />Submitting…</>
                : <>Complete Registration <svg viewBox="0 0 16 10" fill="none" width="14" height="10" aria-hidden="true"><path d="M11 1l4 4-4 4M15 5H1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></>}
            </button>
          )}
        </div>
        <p className="na-rg-form-note">
          By registering you agree to our <a href="#" className="na-rg-form-note__link">Terms &amp; Conditions</a>.
...
export default function Register() {
  ...
  return (
    <div className="na-page na-register-page">
      <NaNavbar />
      {status === "success" && <SuccessScreen fields={submittedData} allConferences={allConferences} onReset={handleReset} />}
