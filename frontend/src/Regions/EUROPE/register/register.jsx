...
import { Navbar } from "../Landingpage/eurohome.jsx";   // ← swap per region
...
const COUNTRY_LIST = (() => {
  ...
  const europe  = mapped.find((c) => c.iso === "us");
  ...
  return europe ? [europe, ...rest] : rest;
})();
...
function CountryDropdown({ value, onChange }) {
  ...
  return (
    <div className="europe-rg-country-dropdown" ref={wrapRef}>
      ...
      {open && (
        <div className="europe-rg-country-list">
          {filtered.length === 0 ? (
            ...
            filtered.map((country) => (
              <div
                ...
                className={`europe-rg-country-option${country.code === value ? " europe-rg-country-option--active" : ""}`}
                onMouseDown={(e) => { e.preventDefault(); handleSelect(country); }}
              >
                <span className="europe-rg-country-option__code">{country.code}</span>
                <span className="europe-rg-country-option__name">{country.name}</span>
              </div>
            ))
          )}
        </div>
...
function Field({ label, required, error, full, children }) {
  return (
    <div className={`europe-rg-field${full ? " europe-rg-field--full" : ""}`}>
      ...
      {children}
      {error && <span className="europe-rg-error-msg">{error}</span>}
    </div>
  );
}
...
function StepIndicator({ currentStep }) {
  ...
  return (
    <div className="europe-rg-step-indicator">
      {steps.map((s, i) => (
        <div key={s.num} className="europe-rg-step-indicator__item">
          ...
          {i < steps.length - 1 && (
            <div className={`europe-rg-step-indicator__line ${currentStep > s.num ? "done" : ""}`} />
          )}
        </div>
      ))}
    </div>
...
function RegisterHero() {
  return (
    <section className="europe-rg-hero">
      <div className="europe-rg-hero__glow" />
...
function Step2({ fields, errors, setField }) {
  ...
  return (
    <div className="europe-rg-form-body">
      <div>
        <label className="europe-rg-label" style={{ marginBottom: 12, display: "block" }}>
          Participation Type <span style={{ color: "var(--europe-rg-accent)" }}>*</span>
        </label>
        {errors.speakerType && (
          <span className="europe-rg-error-msg" style={{ marginBottom: 10, display: "block" }}>
            {errors.speakerType}
          </span>
        )}
        <div className="europe-rg-type-toggle">
          <button
            type="button"
            className={`europe-rg-type-btn${fields.speakerType === "physical" ? " europe-rg-type-btn--active" : ""}`}
            onClick={() => handleTypeSwitch("physical")}
          >
            <span className="europe-rg-type-btn__icon">🎤</span>
            <span className="europe-rg-type-btn__label">Physical Speaker</span>
            <span className="europe-rg-type-btn__sub">In-person at venue</span>
          </button>
          <button
            type="button"
            className={`europe-rg-type-btn${fields.speakerType === "virtual" ? " europe-rg-type-btn--active europe-rg-type-btn--virtual" : ""}`}
            onClick={() => handleTypeSwitch("virtual")}
          >
            <span className="europe-rg-type-btn__icon">💻</span>
            <span className="europe-rg-type-btn__label">Virtual Speaker</span>
            <span className="europe-rg-type-btn__sub">Present via Zoom / Airmeet</span>
          </button>
        </div>
      </div>
      ...
      {fields.speakerType && (
        <div>
          <label className="europe-rg-label" style={{ marginBottom: 14, display: "block" }}>
            Choose Package <span style={{ color: "var(--europe-rg-accent)" }}>*</span>
          </label>
          {errors.packageId && (
            <span className="europe-rg-error-msg" style={{ marginBottom: 10, display: "block" }}>
              {errors.packageId}
          ...
          <div className={`europe-rg-pkg-grid${isVirtual ? " europe-rg-pkg-grid--virtual" : ""}`}>
            {activePkgs.map((pkg) => (
              <div
                key={pkg.id}
              ...
              >
                <div className="europe-rg-pkg-card__radio">✓</div>
                {pkg.badge && <div className="europe-rg-pkg-card__badge">{pkg.badge}</div>}
                <div className="europe-rg-pkg-card__header">
                  <div className="europe-rg-pkg-card__icon">{pkg.icon}</div>
                  <div className="europe-rg-pkg-card__name">{pkg.name}</div>
                  <div className="europe-rg-pkg-card__price">${pkg.price.toLocaleString()}</div>
                </div>
                <ul className="europe-rg-pkg-card__benefits">
      ...
      {fields.speakerType === "physical" && (
        <>
          <div className="europe-rg-divider">Additional Attendees</div>
          <div className="europe-rg-companion-box">
            <div className="europe-rg-companion-box__info">
              <div className="europe-rg-companion-box__title">Accompanying Person(s)</div>
              <div className="europe-rg-companion-box__sub">
                Each additional attendee is ${COMPANION_PRICE} · Currently:{" "}
                <strong style={{ color: "var(--europe-rg-accent)" }}>{fields.companions}</strong>
              </div>
            </div>
            <div className="europe-rg-companion-counter">
              <button
                ...
                type="button"
              >−</button>
              <span className="europe-rg-counter-val">{fields.companions}</span>
              <button
                className="europe-rg-counter-btn"
      ...
      <div className="europe-rg-total-bar">
        <div className="europe-rg-total-bar__left">
          <div className="europe-rg-total-bar__label">Estimated Total</div>
        ...
        <div className="europe-rg-total-bar__amount">${total.toLocaleString()}</div>
      </div>
    </div>
  );
}
    ...
    <div className="europe-rg-coupon-box">
      ...
      {status === "applied" ? (
        ...
        <div className={`europe-rg-coupon-row${shake ? " europe-rg-coupon-row--shake" : ""}`}>
          ...
          <button
            type="button"
            className="europe-rg-coupon-btn"
            onClick={handleApply}
            disabled={!input.trim()}
          >
            Apply
      ...
      {status === "error" && (
        <span className="europe-rg-error-msg" style={{ marginTop: 6, display: "block" }}>
    ...
    <div className="europe-rg-form-body">
      <div className="europe-rg-review-section">
        <div className="europe-rg-review-section__head">
          <span className="europe-rg-review-section__title">Personal Details</span>
          <button className="europe-rg-edit-btn" onClick={() => onEditStep(1)}>Edit</button>
        </div>
        <div className="europe-rg-review-grid">
          {[
            ...
            <div key={label} className="europe-rg-review-row">
      ...
      <div className="europe-rg-review-section">
        <div className="europe-rg-review-section__head">
          <span className="europe-rg-review-section__title">Conference</span>
          <button className="europe-rg-edit-btn" onClick={() => onEditStep(1)}>Edit</button>
        </div>
        <div className="europe-rg-review-grid">
          <div className="europe-rg-review-row">
            <span className="europe-rg-review-label">Region</span>
            <span className="europe-rg-review-value">{regionLabel}</span>
          </div>
          <div className="europe-rg-review-row europe-rg-review-row--full">
            <span className="europe-rg-review-label">Conference</span>
            <span className="europe-rg-review-value">
      ...
      <div className="europe-rg-review-section">
        <div className="europe-rg-review-section__head">
          <span className="europe-rg-review-section__title">Package &amp; Companions</span>
          <button className="europe-rg-edit-btn" onClick={() => onEditStep(2)}>Edit</button>
        </div>
        <div className="europe-rg-review-grid" style={{ marginBottom: 14 }}>
          <div className="europe-rg-review-row">
            <span className="europe-rg-review-label">Participation Type</span>
            <span className="europe-rg-review-value" style={{ textTransform: "capitalize" }}>
              {fields.speakerType || "—"}
            </span>
          </div>
        </div>
        {pkg && (
          <div className="europe-rg-review-pkg" style={{ marginBottom: 14 }}>
            ...
            <span className="europe-rg-review-pkg__icon">{pkg.icon}</span>
        ...
        <div className="europe-rg-review-grid">
          <div className="europe-rg-review-row">
            <span className="europe-rg-review-label">Companions</span>
            <span className="europe-rg-review-value">
              {isVirtual
                ? "N/A (virtual)"
                : fields.companions === 0
...
function SuccessScreen({ fields, allConferences, onReset }) {
  ...
  return (
    <section className="europe-rg-result europe-rg-result--success">
      <div className="europe-rg-result__card">
        <div className="europe-rg-result__icon">✓</div>
        ...
        <div className="europe-rg-result__summary">
          {rows.map(([label, value]) => (
            <div key={label} className="europe-rg-result__row">
...
function FailScreen({ onRetry }) {
  return (
    <section className="europe-rg-result europe-rg-result--fail">
      <div className="europe-rg-result__card">
        <div className="europe-rg-result__icon">✕</div>
...
function RegistrationForm({ onSuccess, onFail }) {
  ...
  return (
    <div className="europe-rg-form-card">
      ...
      <div className="europe-rg-form-footer">
        <div className="europe-rg-form-footer__actions">
          {step > 1 && (
            <button className="europe-rg-back-btn" onClick={goBack}>← Back</button>
          )}
          {step < 3 ? (
            <button className="europe-rg-submit-btn" onClick={goNext}>Continue →</button>
          ) : (
            <button
              className="europe-rg-submit-btn"
...
/* ═══════════════════════════════════════════════════════════
   ROOT — scoped under .europe-register-page
   ═══════════════════════════════════════════════════════════ */
