...
export function getConferencesForRegion(regionId) {
  // ✅ Guard: ensure conferences is always an array
...
export async function submitRegistration(fields, allConferences) {
  // ✅ Guard: ensure allConferences is always a safe array
  ...
  const payload = {
    ...
    CompanionCost:`$${fields.speakerType === "virtual" ? 0 : (fields.companions || 0) * COMPANION_PRICE}`,
    Coupon:       fields.couponCode   || "NONE",
    Discount:     fields.discount > 0 ? `-$${fields.discount}` : "$0",
    TotalAmount:  `$${total}`,
  };

  // ✅ no-cors fetch to Google Apps Script — never throws on network success
  // If this line itself throws, it means a JS runtime error above — now guarded
